...
bool DuelClient::StartClient(unsigned int ip, unsigned short port,
	...
	bufferevent_setcb(client_bev, ClientRead, NULL, ClientEvent,
			(void*) create_game);
	if (bufferevent_socket_connect(client_bev, (sockaddr*) &sin, sizeof(sin))
			< 0) {
	...
	if (!create_game) {
		timeval timeout = { 5, 0 };
		event* resp_event = event_new(client_base, 0, EV_TIMEOUT,
				ConnectTimeout, 0);
...
void DuelClient::ClientEvent(bufferevent *bev, short events, void *ctx) {
	if (events & BEV_EVENT_CONNECTED) {
		...
		if (create_game) {
			...
			BufferIO::CopyWStr(mainGame->ebServerName->getText(), cscg.name,
					20);
			BufferIO::CopyWStr(mainGame->ebServerPass->getText(), cscg.pass,
					20);
			...
			cscg.info.lflist = mainGame->cbLFlist->getItemData(
					mainGame->cbLFlist->getSelected());
			cscg.info.enable_priority =
					mainGame->chkEnablePriority->isChecked();
		...
		if (!is_closing) {
			if (connect_state == 0x1) {
				...
				mainGame->btnJoinCancel->setEnabled(true);
				mainGame->gMutex.Lock();
				mainGame->env->addMessageBox(L"",
						dataManager.GetSysString(1400));
				mainGame->gMutex.Unlock();
				...
				if (!mainGame->dInfo.isStarted && !mainGame->is_building) {
					...
					if (events & BEV_EVENT_EOF)
						mainGame->env->addMessageBox(L"",
								dataManager.GetSysString(1401));
					else
						mainGame->env->addMessageBox(L"",
								dataManager.GetSysString(1402));
					mainGame->gMutex.Unlock();
				} else {
					mainGame->gMutex.Lock();
					mainGame->env->addMessageBox(L"",
							dataManager.GetSysString(1502));
					mainGame->btnCreateHost->setEnabled(true);
					mainGame->btnJoinHost->setEnabled(true);
					mainGame->btnJoinCancel->setEnabled(true);
					mainGame->gMutex.Unlock();
...
void DuelClient::HandleSTOCPacketLan(char* data, unsigned int len) {
	...
	case STOC_ERROR_MSG: {
		...
		case ERRMSG_JOINERROR: {
			...
			if (pkt->code == 0)
				mainGame->env->addMessageBox(L"",
						dataManager.GetSysString(1403));
			else if (pkt->code == 1)
				mainGame->env->addMessageBox(L"",
						dataManager.GetSysString(1404));
			else if (pkt->code == 2)
				mainGame->env->addMessageBox(L"",
						dataManager.GetSysString(1405));
			mainGame->gMutex.Unlock();
			event_base_loopbreak(client_base);
			break;
		}
		case ERRMSG_DECKERROR: {
			...
			if (pkt->code == 1)
				mainGame->env->addMessageBox(L"",
						dataManager.GetSysString(1406));
			else {
				wchar_t msgbuf[256];
				myswprintf(msgbuf, dataManager.GetSysString(1407),
						dataManager.GetName(pkt->code));
		...
		case ERRMSG_VERERROR: {
			...
			mainGame->gMutex.Lock();
			wchar_t msgbuf[256];
			myswprintf(msgbuf, dataManager.GetSysString(1411),
					pkt->code >> 12, (pkt->code >> 4) & 0xff, pkt->code & 0xf);
			mainGame->env->addMessageBox(L"", msgbuf);
			mainGame->gMutex.Unlock();
			event_base_loopbreak(client_base);
			break;
	...
	case STOC_CHANGE_SIDE: {
		...
		mainGame->deckBuilder.pre_extrac =
				deckManager.current_deck.extra.size();
	...
	case STOC_JOIN_GAME: {
		...
		for (auto lit = deckManager._lfList.begin();
				lit != deckManager._lfList.end(); ++lit)
	...
	case STOC_DUEL_START: {
		...
		if (!mainGame->dInfo.isTag) {
			...
			if (selftype != 1) {
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[0]->getText(),
						mainGame->dInfo.hostname, 20);
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[1]->getText(),
						mainGame->dInfo.clientname, 20);
			} else {
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[1]->getText(),
						mainGame->dInfo.hostname, 20);
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[0]->getText(),
			...
			if (selftype > 1 && selftype < 4) {
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[2]->getText(),
						mainGame->dInfo.hostname, 20);
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[3]->getText(),
						mainGame->dInfo.hostname_tag, 20);
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[0]->getText(),
						mainGame->dInfo.clientname, 20);
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[1]->getText(),
						mainGame->dInfo.clientname_tag, 20);
			} else {
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[0]->getText(),
						mainGame->dInfo.hostname, 20);
				BufferIO::CopyWStr(mainGame->stHostPrepDuelist[1]->getText(),
	...
	case STOC_HS_PLAYER_ENTER: {
		...
		if (mainGame->dInfo.isTag) {
			if (pkt->pos == 0)
				...
				BufferIO::CopyWStr(pkt->name, mainGame->dInfo.clientname_tag,
						20);
		} else {
			if (pkt->pos == 0)
				BufferIO::CopyWStr(pkt->name, mainGame->dInfo.hostname, 20);
	...
	case STOC_HS_PLAYER_CHANGE: {
		...
		if (state < 8) {
			mainGame->soundEffectPlayer->doPlayerEnterEffect();
			wchar_t* prename =
					(wchar_t*) mainGame->stHostPrepDuelist[pos]->getText();
			...
			watching++;
			wchar_t watchbuf[32];
			myswprintf(watchbuf, L"%ls%d",
					dataManager.GetSysString(1253), watching);
			mainGame->stHostPrepDuelist[pos]->setText(L"");
			mainGame->chkHostPrepReady[pos]->setChecked(false);
...
int DuelClient::ClientAnalyze(char * msg, unsigned int len) {
	...
	if (!mainGame->dInfo.isReplay && mainGame->dInfo.curMsg != MSG_WAITING
			&& mainGame->dInfo.curMsg != MSG_CARD_SELECTED) {
	...
	case MSG_HINT: {
		...
		case HINT_OPSELECTED: {
			myswprintf(textBuffer, dataManager.GetSysString(1510),
					dataManager.GetDesc(data));
			mainGame->lstLog->addItem(textBuffer);
			mainGame->logParam.push_back(0);
			mainGame->gMutex.Lock();
			mainGame->SetStaticText(mainGame->stACMessage, 310,
					mainGame->textFont, textBuffer);
		...
		case HINT_RACE: {
			myswprintf(textBuffer, dataManager.GetSysString(1511),
					dataManager.FormatRace(data));
			mainGame->lstLog->addItem(textBuffer);
			mainGame->logParam.push_back(0);
			mainGame->gMutex.Lock();
			mainGame->SetStaticText(mainGame->stACMessage, 310,
		...
		case HINT_ATTRIB: {
			myswprintf(textBuffer, dataManager.GetSysString(1511),
					dataManager.FormatAttribute(data));
			mainGame->lstLog->addItem(textBuffer);
			mainGame->logParam.push_back(0);
			mainGame->gMutex.Lock();
			mainGame->SetStaticText(mainGame->stACMessage, 310,
	...
	case MSG_WIN: {
		...
		if (player == 2)
			...
			if (match_kill)
				myswprintf(vic_buf, dataManager.GetVictoryString(0x20),
						dataManager.GetName(match_kill));
			else if (type < 0x10)
				myswprintf(vic_buf, L"[%ls] %ls",
						mainGame->dInfo.clientname, dataManager.GetVictoryString(type));
			else
			...
			if (match_kill)
				...
				myswprintf(vic_buf, L"[%ls] %ls",
						mainGame->dInfo.hostname, dataManager.GetVictoryString(type));
			else
				myswprintf(vic_buf, L"%ls", dataManager.GetVictoryString(type));
			mainGame->dInfo.vic_string = vic_buf;
		}
	...
	case MSG_START: {
		...
		mainGame->dInfo.lp[mainGame->LocalPlayer(0)] = BufferIO::ReadInt32(
				pbuf);
		mainGame->dInfo.lp[mainGame->LocalPlayer(1)] = BufferIO::ReadInt32(
				pbuf);
	...
	case MSG_SELECT_EFFECTYN: {
		...
		myswprintf(textBuffer, dataManager.GetSysString(200),
				dataManager.FormatLocation(l, s), dataManager.GetName(code));
		mainGame->gMutex.Lock();
		mainGame->SetStaticText(mainGame->stQMessage, 310, mainGame->textFont,
				textBuffer);
	...
	case MSG_SELECT_YESNO: {
		...
		mainGame->gMutex.Lock();
		mainGame->SetStaticText(mainGame->stQMessage, 310, mainGame->textFont,
				(wchar_t*) dataManager.GetDesc(desc));
		mainGame->PopupElement(mainGame->wQuery);
		mainGame->gMutex.Unlock();
		return false;
	}
	case MSG_SELECT_OPTION: {
		...
		for (int i = 0; i < count; ++i)
			mainGame->dField.select_options.push_back(
					BufferIO::ReadInt32(pbuf));
		mainGame->gMutex.Lock();
		mainGame->SetStaticText(mainGame->stOptions, 310, mainGame->textFont,
				(wchar_t*) dataManager.GetDesc(
						mainGame->dField.select_options[0]));
	...
	case MSG_SELECT_CARD: {
		...
		std::sort(mainGame->dField.selectable_cards.begin(),
				mainGame->dField.selectable_cards.end(),
				ClientCard::client_card_sort);
		if (select_hint)
			myswprintf(textBuffer, L"%ls(%d-%d)",
					dataManager.GetDesc(select_hint), mainGame->dField.select_min, mainGame->dField.select_max);
		else
			myswprintf(textBuffer, L"%ls(%d-%d)",
					dataManager.GetSysString(560), mainGame->dField.select_min, mainGame->dField.select_max);
		select_hint = 0;
		if (panelmode) {
			mainGame->gMutex.Lock();
	...
	case MSG_SELECT_CHAIN: {
		...
		if (!forced
				&& (mainGame->ignore_chain
						|| ((count == 0 || specount == 0)
								&& !mainGame->always_chain))) {
			SetResponseI(-1);
		...
		if (panelmode) {
			...
			mainGame->dField.selectable_cards =
					mainGame->dField.activatable_cards;
			std::sort(mainGame->dField.selectable_cards.begin(),
					mainGame->dField.selectable_cards.end());
			auto eit = std::unique(mainGame->dField.selectable_cards.begin(),
					mainGame->dField.selectable_cards.end());
			mainGame->dField.selectable_cards.erase(eit,
					mainGame->dField.selectable_cards.end());
			...
			if (!forced) {
				if (count == 0)
					myswprintf(textBuffer, L"%ls\n%ls",
							dataManager.GetSysString(201), dataManager.GetSysString(202));
				else
					myswprintf(textBuffer, L"%ls\n%ls",
	...
	case MSG_SELECT_POSITION: {
		...
		if (positions == 0x1 || positions == 0x2 || positions == 0x4
				|| positions == 0x8) {
	...
	case MSG_SELECT_TRIBUTE: {
		/*int selecting_player = */BufferIO::ReadInt8(pbuf);
		mainGame->dField.select_cancelable =
				BufferIO::ReadInt8(pbuf) ? true : false;
		mainGame->dField.select_min = BufferIO::ReadInt8(pbuf);
		mainGame->dField.select_max = BufferIO::ReadInt8(pbuf);
		int count = BufferIO::ReadInt8(pbuf);
		mainGame->dField.selectable_cards.clear();
	...
	case MSG_SELECT_COUNTER: {
		...
		myswprintf(textBuffer, dataManager.GetSysString(204),
				mainGame->dField.select_counter_count, dataManager.GetCounterName(mainGame->dField.select_counter_type));
		mainGame->gMutex.Lock();
		mainGame->stHintMsg->setText(textBuffer);
		mainGame->stHintMsg->setVisible(true);
		mainGame->gMutex.Unlock();
	...
	case MSG_SELECT_SUM: {
		...
		std::sort(mainGame->dField.selectsum_all.begin(),
				mainGame->dField.selectsum_all.end(),
				ClientCard::client_card_sort);
		mainGame->dField.CheckSelectSum();
		if (select_hint)
			myswprintf(textBuffer, L"%ls(%d)",
					dataManager.GetDesc(select_hint), mainGame->dField.select_sumval);
		else
			myswprintf(textBuffer, L"%ls(%d)",
					dataManager.GetSysString(560), mainGame->dField.select_sumval);
		select_hint = 0;
		if (panelmode) {
			mainGame->wCardSelect->setText(textBuffer);
			mainGame->dField.ShowSelectCard();
...
mainGame		->dField.sort_list.clear();
	...
	case MSG_CONFIRM_CARDS: {
			...
			if (l & 0x41) {
				if (count == 1) {
					...
					if ((l == LOCATION_DECK) && mainGame->dField.deck_reversed)
						...
						pcard->dRot = irr::core::vector3df(0, 3.14159f / 5.0f,
								0);
					pcard->is_moving = true;
					pcard->aniFrame = 5;
					mainGame->WaitFrameSignal(45);
			...
			for (size_t i = 0; i < field_confirm.size(); ++i) {
				...
				if (l == LOCATION_HAND) {
					...
					if (pcard->position == POS_FACEDOWN_ATTACK)
						...
						pcard->dRot = irr::core::vector3df(3.14159f / 5.0f, 0,
								0);
					pcard->is_moving = true;
					pcard->aniFrame = 5;
		...
		if (panel_confirm.size()) {
			std::sort(panel_confirm.begin(), panel_confirm.end(),
					ClientCard::client_card_sort);
			mainGame->gMutex.Lock();
			mainGame->dField.selectable_cards = panel_confirm;
			myswprintf(textBuffer, dataManager.GetSysString(208),
					panel_confirm.size());
			mainGame->wCardSelect->setText(textBuffer);
	...
	case MSG_SHUFFLE_DECK: {
		...
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			...
			if (rev) {
				for (size_t i = 0; i < mainGame->dField.deck[player].size();
						++i)
					mainGame->dField.MoveCard(mainGame->dField.deck[player][i],
							10);
		...
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			for (int i = 0; i < 5; ++i) {
				for (auto cit = mainGame->dField.deck[player].begin();
						cit != mainGame->dField.deck[player].end(); ++cit) {
					(*cit)->dPos = irr::core::vector3df(
							rand() * 0.4f / RAND_MAX - 0.2f, 0, 0);
	...
	case MSG_SHUFFLE_HAND: {
		...
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			...
			if (player == 1 && !mainGame->dInfo.isReplay
					&& !mainGame->dInfo.isSingleMode) {
				bool flip = false;
				for (auto cit = mainGame->dField.hand[player].begin();
						cit != mainGame->dField.hand[player].end(); ++cit)
					if ((*cit)->code) {
						(*cit)->dPos = irr::core::vector3df(0, 0, 0);
						(*cit)->dRot = irr::core::vector3df(1.322f / 5,
								3.1415926f / 5, 0);
						(*cit)->is_moving = true;
	...
	case MSG_SWAP_GRAVE_DECK: {
		...
		if (mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
			...
			for (auto cit = mainGame->dField.grave[player].begin();
					cit != mainGame->dField.grave[player].end(); ++cit)
				(*cit)->location = LOCATION_GRAVE;
			for (auto cit = mainGame->dField.deck[player].begin();
					cit != mainGame->dField.deck[player].end();) {
	...
	case MSG_SHUFFLE_SET_CARD: {
		...
		for (int i = 0; i < count; ++i) {
			...
			mc[i]->SetCode(0);
			if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
				mc[i]->dPos = irr::core::vector3df(
						(3.95f - mc[i]->curPos.X) / 10, 0, 0.05f);
		...
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			for (int i = 0; i < count; ++i) {
				...
				for (cit = mc[i]->overlayed.begin();
						cit != mc[i]->overlayed.end(); ++cit)
					mainGame->dField.MoveCard(*cit, 10);
			}
			mainGame->WaitFrameSignal(11);
		}
	...
	case MSG_NEW_TURN: {
		...
		int player = mainGame->LocalPlayer(BufferIO::ReadInt8(pbuf));
		mainGame->dInfo.turn++;
		if (!mainGame->dInfo.isTag && mainGame->dInfo.turn == 5
		...
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			myswprintf(mainGame->dInfo.strTurn, L"Turn:%d",
					mainGame->dInfo.turn);
	...
	case MSG_MOVE: {
		...
		if (pl == 0) {
			...
			if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
				...
				mainGame->gMutex.Unlock();
				mainGame->dField.GetCardLocation(pcard, &pcard->curPos,
						&pcard->curRot);
			...
			if (code != 0 && pcard->code != code)
				pcard->SetCode(code);
			pcard->ClearTarget();
			for (auto eqit = pcard->equipped.begin();
					eqit != pcard->equipped.end(); ++eqit)
				(*eqit)->equipTarget = 0;
			if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			...
			if (!(pl & 0x80) && !(cl & 0x80)) {
					...
					if (pl == cl && pc == cc && (cl & 0x71)) {
						...
						if (cl == 0x4 && pcard->overlayed.size() > 0) {
							...
							for (size_t i = 0; i < pcard->overlayed.size(); ++i)
								mainGame->dField.MoveCard(pcard->overlayed[i],
										10);
							mainGame->gMutex.Unlock();
							mainGame->WaitFrameSignal(10);
						}
						if (cl == 0x2) {
							mainGame->gMutex.Lock();
							for (size_t i = 0;
									i < mainGame->dField.hand[cc].size(); ++i)
								mainGame->dField.MoveCard(
										mainGame->dField.hand[cc][i], 10);
							mainGame->gMutex.Unlock();
						} else {
							mainGame->gMutex.Lock();
							mainGame->dField.MoveCard(pcard, 10);
							if (pl == 0x2)
								for (size_t i = 0;
										i < mainGame->dField.hand[pc].size();
										++i)
									mainGame->dField.MoveCard(
											mainGame->dField.hand[pc][i], 10);
							mainGame->gMutex.Unlock();
						}
						mainGame->WaitFrameSignal(5);
					}
				...
				if(mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping) {
					...
					if (olcard->location == 0x4) {
						...
						if (pl == 0x2)
							for (size_t i = 0;
									i < mainGame->dField.hand[pc].size(); ++i)
								mainGame->dField.MoveCard(
										mainGame->dField.hand[pc][i], 10);
						mainGame->gMutex.Unlock();
						mainGame->WaitFrameSignal(5);
				...
				ClientCard* olcard = mainGame->dField.GetCard(pc, pl & 0x7f,
						ps);
				ClientCard* pcard = olcard->overlayed[pp];
				if (mainGame->dInfo.isReplay
						&& mainGame->dInfo.isReplaySkiping) {
					olcard->overlayed.erase(
							olcard->overlayed.begin() + pcard->sequence);
				...
				ClientCard* olcard1 = mainGame->dField.GetCard(pc, pl & 0x7f,
						ps);
				ClientCard* pcard = olcard1->overlayed[pp];
				ClientCard* olcard2 = mainGame->dField.GetCard(cc, cl & 0x7f,
						cs);
				if (mainGame->dInfo.isReplay
						&& mainGame->dInfo.isReplaySkiping) {
					olcard1->overlayed.erase(
							olcard1->overlayed.begin() + pcard->sequence);
	...
	case MSG_SUMMONING: {
		...
		/*int cp = */BufferIO::ReadInt8(pbuf);
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			myswprintf(event_string, dataManager.GetSysString(1603),
					dataManager.GetName(code));
	...
	case MSG_SPSUMMONING: {
		...
		/*int cp = */BufferIO::ReadInt8(pbuf);
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			myswprintf(event_string, dataManager.GetSysString(1605),
					dataManager.GetName(code));
			mainGame->showcardcode = code;
	...
	case MSG_FLIPSUMMONING: {
		...
		pcard->position = cp;
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			myswprintf(event_string, dataManager.GetSysString(1607),
					dataManager.GetName(code));
			mainGame->dField.MoveCard(pcard, 10);
			mainGame->WaitFrameSignal(11);
			mainGame->showcardcode = code;
	...
	case MSG_CHAINING: {
		...
		mainGame->dField.GetChainLocation(cc, cl, cs,
				&mainGame->dField.current_chain.chain_pos);
		mainGame->dField.current_chain.solved = false;
		int chc = 0;
		for (size_t i = 0; i < mainGame->dField.chains.size(); ++i) {
			if (cl == 0x10 || cl == 0x20) {
				if (mainGame->dField.chains[i].controler == cc
						&& mainGame->dField.chains[i].location == cl)
					chc++;
			} else {
				if (mainGame->dField.chains[i].controler == cc
						&& mainGame->dField.chains[i].location == cl
						&& mainGame->dField.chains[i].sequence == cs)
					chc++;
			}
		}
		if (cl == LOCATION_HAND)
	...
	case MSG_CHAINED: {
		...
		if (mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping)
			return true;
		myswprintf(event_string, dataManager.GetSysString(1609),
				dataManager.GetName(mainGame->dField.current_chain.code));
		mainGame->gMutex.Lock();
	...
	case MSG_RANDOM_SELECTED: {
		...
		for (int i = 0; i < count; ++i) {
			...
			if ((l & 0x80) > 0)
				pcards[i] =
						mainGame->dField.GetCard(c, l & 0x7f, s)->overlayed[ss];
			else
	...
	case MSG_BECOME_TARGET: {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (pcard->location & LOCATION_ONFIELD) {
				...
				mainGame->WaitFrameSignal(30);
			myswprintf(textBuffer, dataManager.GetSysString(1610),
					dataManager.GetName(pcard->code), dataManager.FormatLocation(l, s), s);
			mainGame->lstLog->addItem(textBuffer);
			mainGame->logParam.push_back(pcard->code);
			pcard->is_highlighting = false;
		}
	...
	case MSG_ADD_COUNTER: {
		...
		if (mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping)
			return true;
		myswprintf(textBuffer, dataManager.GetSysString(1617),
				dataManager.GetName(pc->code), count, dataManager.GetCounterName(type));
	...
	case MSG_REMOVE_COUNTER: {
		...
		if (mainGame->dInfo.isReplay && mainGame->dInfo.isReplaySkiping)
			return true;
		myswprintf(textBuffer, dataManager.GetSysString(1618),
				dataManager.GetName(pc->code), count, dataManager.GetCounterName(type));
		pc->is_highlighting = true;
		mainGame->gMutex.Lock();
		mainGame->stACMessage->setText(textBuffer);
	...
	case MSG_ATTACK: {
		...
		float sy;
		if (ld != 0) {
			mainGame->dField.attack_target = mainGame->dField.GetCard(cd, ld,
					sd);
			myswprintf(event_string, dataManager.GetSysString(1619),
					dataManager.GetName(mainGame->dField.attacker->code), dataManager.GetName(mainGame->dField.attack_target->code));
			...
			sy = (float) sqrt((xa - xd) * (xa - xd) + (ya - yd) * (ya - yd))
					/ 2;
			mainGame->atk_t = vector3df((xa + xd) / 2, (ya + yd) / 2, 0);
			if (ca == 0)
				mainGame->atk_r = vector3df(0, 0, -atan((xd - xa) / (yd - ya)));
			else
				mainGame->atk_r = vector3df(0, 0,
						3.1415926 - atan((xd - xa) / (yd - ya)));
		} else {
			myswprintf(event_string, dataManager.GetSysString(1620),
					dataManager.GetName(mainGame->dField.attacker->code));
			float xa = mainGame->dField.attacker->curPos.X;
			float ya = mainGame->dField.attacker->curPos.Y;
			float xd = 3.95f;
			float yd = 3.5f;
	...
	case MSG_ATTACK_DISABLED: {
		myswprintf(event_string, dataManager.GetSysString(1621),
				dataManager.GetName(mainGame->dField.attacker->code));
		return true;
	}
	...
	case MSG_MISSED_EFFECT: {
		BufferIO::ReadInt32(pbuf);
		unsigned int code = (unsigned int) BufferIO::ReadInt32(pbuf);
		myswprintf(textBuffer, dataManager.GetSysString(1622),
				dataManager.GetName(code));
		mainGame->lstLog->addItem(textBuffer);
		mainGame->logParam.push_back(code);
		return true;
	...
	case MSG_TOSS_COIN: {
		...
		for (int i = 0; i < count; ++i) {
			...
			*pwbuf++ = L'[';
			BufferIO::CopyWStrRef(dataManager.GetSysString(res ? 60 : 61),
					pwbuf, 256);
			*pwbuf++ = L']';
		}
		*pwbuf = 0;
...
//		for(int i = 0, filter = 0x1; i < 23; ++i, filter <<= 1) {
	...
	case MSG_TAG_SWAP: {
		...
		if (!mainGame->dInfo.isReplay || !mainGame->dInfo.isReplaySkiping) {
			...
			for (auto cit = mainGame->dField.extra[player].begin();
					cit != mainGame->dField.extra[player].end(); ++cit) {
				if (player == 0)
					(*cit)->dPos.Y = 0.4f;
				else
					(*cit)->dPos.Y = -0.6f;
		...
		for (int p = 0; p < 2; ++p) {
			...
			for (int seq = 0; seq < 5; ++seq) {
				...
				if (val) {
					...
					mainGame->dField.GetCardLocation(ccard, &ccard->curPos,
							&ccard->curRot, true);
					val = BufferIO::ReadInt8(pbuf);
					if (val) {
						for (int xyz = 0; xyz < val; ++xyz) {
							...
							mainGame->dField.GetCardLocation(xcard,
									&xcard->curPos, &xcard->curRot, true);
						}
					}
				}
			}
...
void DuelClient::BeginRefreshHost() {
	...
	timeval timeout = { 3, 0 };
	resp_event = event_new(broadev, reply, EV_TIMEOUT | EV_READ | EV_PERSIST,
			BroadcastReply, broadev);
	event_add(resp_event, &timeout);
...
void DuelClient::BroadcastReply(evutil_socket_t fd, short events, void * arg) {
	if (events & EV_TIMEOUT) {
		...
		if (!is_closing && pHP->identifier == NETWORK_SERVER_ID
				...
				&& remotes.find(ipaddr) == remotes.end()) {
			mainGame->gMutex.Lock();
			...
			hoststr.append(L"][");
			if (pHP->host.draw_count == 1 && pHP->host.start_hand == 5
					&& pHP->host.start_lp == 8000 && !pHP->host.no_check_deck
					&& !pHP->host.no_shuffle_deck && !pHP->host.enable_priority)
				hoststr.append(dataManager.GetSysString(1280));
			else
				hoststr.append(dataManager.GetSysString(1281));
			hoststr.append(L"]");
			wchar_t gamename[20];
